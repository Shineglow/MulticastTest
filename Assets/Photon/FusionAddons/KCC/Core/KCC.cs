	...
	public sealed partial class KCC : NetworkAreaOfInterestBehaviour, IBeforeAllTicks, IAfterTick
	{
		...
		private void ProcessPhysicsQuery(KCCData data)
		{
			...
			if (_settings.CollisionLayerMask != 0 && _collider.IsSpawned == true)
			{
				...
				OverlapCapsule(_extendedOverlapInfo, data, data.TargetPosition, _settings.Radius, _settings.Height, baseOverlapQueryExtent, _settings.CollisionLayerMask, QueryTriggerInteraction.Collide);
		...
		private Vector3 DepenetrateSingle(KCCOverlapInfo overlapInfo, KCCData data, Vector3 basePosition, Vector3 targetPosition, bool probeGrounding, int maxSteps)
		{
			...
			KCCUtility.GetPositionAndRotation(hit.Transform, out hit.CachedPosition, out hit.CachedRotation);
		...
		private void TryStepUp(KCCOverlapInfo overlapInfo, KCCData data)
		{
			...
			else
			{
				...
				if (checkDesiredDistance > 0.001f)
				{
					...
					if (targetDistance / checkDesiredDistance < _settings.StepMinPushBack)
					{
						tryStepUp = true;
					}
				}
			...
			if (_settings.StepGroundCheckRadiusScale < 1.0f)
			{
				// Ground check can be done with smaller radius to compensate normal on edges.
				checkRadius = _settings.Radius * _settings.StepGroundCheckRadiusScale;
				checkPosition += combinedCheckDirectionXZ * (_settings.Radius - checkRadius);
			}
			...
			if (_settings.StepRequiresGroundTarget == true && data.IsSteppingUp == false && data.WasSteppingUp == false)
			{
			...
			float remainingDistance = Mathf.Clamp((desiredDistance - traveledDistance) * _settings.StepSpeed, 0.0f, maxStepHeight);
		...
		private void UpdateHits(KCCData data, KCCOverlapInfo extendedOverlapInfo, EKCCHitsOverlapQuery overlapQuery)
		{
			...
			switch (overlapQuery)
			{
				case EKCCHitsOverlapQuery.Default: { reuseOverlapInfo = extendedOverlapInfo != null && extendedOverlapInfo.AllWithinExtent() == true; break; }
				case EKCCHitsOverlapQuery.Reuse:   { reuseOverlapInfo = extendedOverlapInfo != null;                                                  break; }
